...
rollback() {
  ...
  if [ -n "$BACKUP_DIR" ] && [ -d "$BACKUP_DIR" ]; then
    ...
    echo -e "  ${Y}[!]${NC} Code restauré depuis la sauvegarde"
  fi
...
if [ ! -f "$SRC/app.py" ] || [ ! -f "$SRC/core/config.py" ] || [ ! -f "$SRC/blueprints/auth.py" ]; then
  echo -e "${R}[✗] Sources incomplètes (app.py, core/config.py ou blueprints/auth.py manquant).${NC}"
  exit 1
fi
...
echo -e "  ${G}✓${NC} $UPDATED éléments applicatifs"
